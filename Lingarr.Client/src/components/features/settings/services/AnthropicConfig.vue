<template>
    <div class="flex flex-col space-y-2">
        ...
        <SelectComponent
            ...
            :options="[{ label: 'Claude 3.5 Sonnet', value: 'claude-3-5-sonnet-20240620' }]" />
...
const aiModel = computed({
    ...
    set: (newValue: string) => {
        settingsStore.updateSetting(SETTINGS.ANTHROPIC_MODEL, newValue)
        emit('save')
    }
})
...
const apiKey = computed({
    ...
    set: (newValue: string) => {
        settingsStore.updateSetting(SETTINGS.ANTHROPIC_API_KEY, newValue)
        emit('save')
    }
})
