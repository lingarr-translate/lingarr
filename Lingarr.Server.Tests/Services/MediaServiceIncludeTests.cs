...
public class MediaServiceIncludeTests : IDisposable
{
    ...
    private static Movie CreateMovie(int radarrId, string title, bool exclude = true) => new()
    {
        ...
        ExcludeFromTranslation = exclude
    };
    ...
    [Fact]
    public async Task SetInclude_Movie_IncludeSetsExcludeToFalse()
    {
        ...
        Assert.False(updated!.ExcludeFromTranslation);
    }

    [Fact]
    public async Task SetInclude_Movie_ExcludeSetsExcludeToTrue()
    {
        ...
        _context.ChangeTracker.Clear();
        var updated = await _context.Movies.FindAsync(movie.Id);
        Assert.True(updated!.ExcludeFromTranslation);
    }

    [Fact]
    public async Task SetInclude_Show_IncludeSetsExcludeToFalse()
    {
    ...
    [Fact]
    public async Task SetInclude_Season_IncludeSetsExcludeToFalse()
    {
    ...
    [Fact]
    public async Task SetInclude_Episode_IncludeSetsExcludeToFalse()
    {
    ...
    public async Task SetIncludeAll_Movies_IncludesAll()
    {
        ...
        Assert.All(movies, m => Assert.False(m.ExcludeFromTranslation));
    }

    [Fact]
    public async Task SetIncludeAll_Movies_ExcludesAll()
    {
        _context.Movies.AddRange(
        ...
        _context.ChangeTracker.Clear();
        var movies = await _context.Movies.ToListAsync();
        Assert.All(movies, m => Assert.True(m.ExcludeFromTranslation));
    }
    ...
    public async Task SetIncludeAll_Shows_CascadesToSeasonsAndEpisodes()
    {
        ...
        Assert.False(updatedShow!.ExcludeFromTranslation);

        var seasons = await _context.Seasons.Where(s => s.ShowId == show.Id).ToListAsync();
        Assert.All(seasons, s => Assert.False(s.ExcludeFromTranslation));

        var episodes = await _context.Episodes.ToListAsync();
        Assert.All(episodes, e => Assert.False(e.ExcludeFromTranslation));
    }

    [Fact]
    public async Task SetIncludeAll_Shows_ExcludeCascadesToSeasonsAndEpisodes()
    {
        ...
        var updatedShow = await _context.Shows.FindAsync(show.Id);
        Assert.True(updatedShow!.ExcludeFromTranslation);

        var updatedSeason = await _context.Seasons.FindAsync(season.Id);
        Assert.True(updatedSeason!.ExcludeFromTranslation);

        var updatedEp = await _context.Episodes.FindAsync(ep.Id);
        Assert.True(updatedEp!.ExcludeFromTranslation);
    }
